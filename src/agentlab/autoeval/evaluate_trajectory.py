...
import argparse
import traceback
from agentlab.autoeval.evaluator import Evaluator
from agentlab.autoeval.clients import CLIENT_DICT
from agentlab.utils.utils import get_trajectory_from_annotation
...
def load_blocks(path: str) -> list[list[str]]:
    ...
    # remove the block that contains "Python version"
    blocks = [b for b in blocks if "Python version" not in b[0]]
...
def main():
    ...
    traj = get_trajectory_from_annotation(args.result_dir)
    # construct traj_info from traj
    actions = [step["action"] for step in traj]
    # remove the last action if it is None
    if actions[-1] is None:
        actions = actions[:-1]
    thinks = [step["think"] for step in traj]
    # remove the last think if it is None
    if thinks[-1] is None:
        thinks = thinks[:-1]
    if "send_msg_to_user" in actions[-1]:
        response = extract_response(actions[-1])
    else:
        response = ""
    image_paths = [
        os.path.join(args.result_dir, f) for f in os.listdir(args.result_dir) 
    ...
    # evaluate trajectory
    log_save_path = os.path.join(f"autoeval/log/{args.model}", args.result_dir.split('/')[-2])
