...
@dataclass
class AugmentedPromptFlags(dp.Flags):
    """
...
BASIC_FLAGS = AugmentedPromptFlags(
    obs=dp.ObsFlags(
        use_html=False,
        use_ax_tree=True,
...
ADVANCED_FLAGS = AugmentedPromptFlags(
    obs=dp.ObsFlags(
        use_html=False,
        use_ax_tree=True,
...
class Experience(dp.PromptElement):
    def __init__(self, experience: list[dict], visible: bool = True) -> None:
        super().__init__(visible=visible)
        self._prompt = f"""\
# Experience
Note: This is a list of sub-goals and its corresponding instructions to achieve the goal. You may refer to this to complete the task.
"""
        for exp in experience:
            self._prompt += f"""\
## {exp["sub_goal"].capitalize()}
...
* When you try to complete certain sub-goals that are in experience or similar to those in experience section, you should refer to the corresponding instrucitons.
