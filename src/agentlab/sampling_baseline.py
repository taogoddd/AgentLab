...
def parse_args():
    ...
    parser.add_argument("--website", type=str, required=True,
                        choices=["shopping", "reddit", "gitlab", "shopping_admin", "map"])
    parser.add_argument("--start_id", type=int, default=0, help="Starting task id")
    parser.add_argument("--end_id", type=int, default=812, help="Ending task id, not greater than max_id in the website tasks")
...
def main():
    ...
    if args.result_dir_id == "":
        result_dir_id = f"sampling_baseline_"+time.strftime("%Y%m%d%H%M%S", time.localtime())
    else:
    ...
    for task_id in new_task_ids:
        try:
            ...
            for i in range(args.num_samples):
                print("*"*50, f"Running task {task_id} sample {i}", "*"*50)
                ...
                try:
                    ...
                    print(f"Error in task {task_id} sample {i}")
                    continue
            
            selected_id = select_best_sample(result_parent_dir=f"results/{result_dir_id}/webarena.{task_id}", sample_ids=valid_sample_ids)

            print("*"*50, f"Selected sample {selected_id} for task {task_id}", "*"*50)

            selected_dir = f"results/{result_dir_id}/webarena.{task_id}/{selected_id}"
            # copy the traj in selected_dir to the result_parent_dir with id 0
            process = Popen([
                "cp", "-r", selected_dir, f"results/{result_dir_id}/webarena.{task_id}/0"
            ])
