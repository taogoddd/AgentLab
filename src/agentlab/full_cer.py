...
def parse_args():
    ...
    parser.add_argument("--skill_path", type=str, default=None, help="Root path to save the learned skills")
...
def main():
    ...
    # init the skill file
    if not os.path.exists(f"{skill_path}"):
        with open(f"{skill_path}", "w") as f:
            json.dump([], f)
