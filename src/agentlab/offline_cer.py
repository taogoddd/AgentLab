...
def main():
    ...
    for i in range(args.num_samples):
        process = Popen([
            ...
            "--result_dir", f"results/rd_explore_{result_dir_id}/webarena.{start_task_id}",
            "--model_name", "azureopenai/"+args.model,
            "--id", str(i),
            "--max_exploration_steps", str(args.max_steps),
            "--use_screenshot", "1"
        ])
        process.wait()
    ...
    for i in range(args.num_samples):
        task_dir = f"results/rd_explore_{result_dir_id}/webarena.{start_task_id}/{i}"
